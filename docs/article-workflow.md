...
- 除外: `.page-meta` / `.disclaimer` / `.map-cta` / `.related` / 「関連情報・参考リンク」h2以降
...
### タイトル（`<title>`）の付け方（2026-09-15 変更）

検索結果では長いタイトルの後半が切れ、「停車駅」「乗り方」など肝心の語が見えなくなる。GSC でも、同じ順位帯で長いタイトルほどクリックされにくい傾向が出たため、次のルールにする。

- `<title>` の本体（「 | 鉄道マップ全国版」を除く）は **40字以内**
- **検索される語を前に置く**: 列車名 → 停車駅 / 乗り方 / 違い / 路線図・地図 / 所要時間 / 料金
- 「完全ガイド」・車両形式（787系 等）・キャッチコピー的な修飾は `<title>` に入れず、**h1 と description に回す**（h1 は長くてよい）
- og:title / twitter:title は `<title>` 本体と同じ文言、JSON-LD `headline` は h1 と同じ文言
- 似た列車がある場合は「違い」を前半に置く（「〇〇 違い」系の検索は順位が取りやすい）

| | 例 |
|---|---|
| ❌ 旧形式（58字） | にちりん・にちりんシーガイア完全ガイド｜博多・大分〜宮崎の停車駅・787系／783系・日豊本線を南下する特急の乗り方 |
| ✅ title（30字） | にちりんシーガイア・にちりん 停車駅と乗り方｜博多〜宮崎空港 |
| ✅ h1（長くてよい） | 旧形式の文言をそのまま使ってよい |

字数の確認: `python3 -c "import re,sys;t=re.search(r'<title>(.*?) \| 鉄道マップ全国版</title>',open(sys.argv[1]).read()).group(1);print(len(t),t)" routes/{SLUG}.html`
...
- **自動生成モード（現在の標準）**: 執筆AIがバルーン本文を生成して `class="speech"` で埋めて公開。
  公開と同時に `docs/balloon-review-queue.md` へ全バルーンを記録し、運営者が2〜3日ごとに
  まとめてレビュー・修正する（修正済みエントリはキューから削除）。
- **指定モード**: 依頼プロンプトでバルーン文言が指定された場合はそれをそのまま使う。

以下は旧運用（draft プレースホルダを運営者が手動で埋める手順）。テンプレの speech--draft
を残したまま公開した場合に参照:

執筆AIが残した `speech speech--draft` バルーンを、運営者が実際に読んでコメントを入れる工程。
