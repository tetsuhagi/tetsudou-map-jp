...
6. `docs/article-template.html` を元に執筆
   ...
   - タイトルは SEO/SNS 最適化して生成（`docs/article-request-template.md` の生成ルール参照）
     ⚠ **`<title>` 本体は40字以内・検索される語（停車駅／乗り方／違い／路線図）を前に**。「〇〇完全ガイド｜…の乗り方」の長い形式は `<title>` に使わず h1 に回す（`article-workflow.md`「タイトル（`<title>`）の付け方」）
   ...
   - **読ませる工夫（エンタメ性）を効かせる**（`voice-guide.md` §6.5）。物語のアーク・意外性のフック・つかみ導入・物語的見出し・締めのリワードで読了率を上げる。
     本体はですます調・事実厳守のまま"構成と語り口"で味付けし、笑いはバルーンに寄せる。**タイプ別濃度を守る**: 歴史/文化/考察コラム=高 ／ 路線ガイド・比較=中（実用情報の整理を最優先した上で味付け） ／ ニュース=低（中立性優先・煽らない）。基準サンプル: `columns/nishitetsu-history.html`。
...
7.5 **アフィリエイトCTA（該当テーマ記事のみ）**: 記事が旅行・宿泊・移動手段比較・観光地アクセス等
   ...
   - **意図ティアで濃度を出し分ける**（`affiliate-links.md`「配置の意図ティア」参照）:
     趣味・スペック解説 → 楽天1ボタン（or 無し）／旅行計画・比較・おすすめ宿系 → **楽天＋じゃらん2ボタン**(`.affiliate-cta--duo`)。
...
- [ ] `<title>` 本体が40字以内で、og:title / twitter:title と同じ文言。「完全ガイド」や車両形式は h1 側にある
