...
| サムネ | なし（サムネ非表示方針継続） | 白背景テキストサムネ（Thread A のスクリプト完成後） |
| バルーン | 自動生成で埋める | 同左 |
...
- [ ] og:image / twitter:image / article-hero が **入っていない**（サムネ非表示方針）
- [ ] アフィリエイトリンク使用時: `<a>` 部分が `docs/affiliate-links.md` の原文と完全一致・1〜2箇所のみ・disclaimer に開示文
