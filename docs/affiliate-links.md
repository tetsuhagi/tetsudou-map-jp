...
記事本文の該当箇所に次の1行を置く。HTMLは書かない。

```html
<!-- GEAR:eye-mask -->
```

そのうえで `python3 scripts/gen_gear_cta.py` を実行すると、マーカー直後にCTAブロックが生成される（**冪等**・何度実行しても同じ）。
商品を入れ替えるときは `data/gear-picks.json` を直して再実行すれば全記事が一括更新される。

- 定義済みID: `mobile-battery` / `charger` / `privacy-filter` / `privacy-filter-phone` / `neck-pillow` / `earplug` / `eye-mask` / `noise-cancelling` / `travel-slippers` / `suitcase` / `timetable-book`
- **1記事に詰め込まない。文脈に合う 4〜6個** に絞る
- 個別商品ページではなく **Amazon検索結果ページ** へリンクする方式（型落ち防止。PA-APIは売上要件があり当面使えないため）

### Amazonアソシエイト運営規約（違反はアカウントに影響）

1. **価格を本文に書かない** — PA-API経由以外での価格表示は規約違反。「約○○円」もNG。「価格はリンク先でご確認ください」とする
2. **Amazonの商品画像を自前で保存して使わない**
3. **「ベストセラー」「Amazon1位」等と断定しない** — ランキングは変動し、時点によって事実と食い違う（景表法・ステマ規制リスク）。「定番として選ばれることが多い」等に留める
4. **使っていない商品を「使ってみた」と書かない** — E-E-A-T的にも逆効果。実体験があるものは体験として、ないものは「こういう場面で必要になる」という**選び方の解説**に徹する
5. 開示: disclaimer に「※ 本記事には広告・アフィリエイトリンクが含まれます。」／CTA脚注は `gear-picks.json` の `_note_text`（「※ 広告（Amazonアソシエイト・リンク）です」）が自動で入る。
   Amazonアソシエイト参加の定型表記は**フッターに全ページ掲載済み**のため記事側には不要
